...
# Set tolerance for difference in total momentum between hemispheres
tolerance = 0.1
...
def main(debug, input_path, event_limit):
    ...
    # Pair up jets based on total momentum
    newevents = []
    for i in range (len(hemispheres)):
        flag = 0
        lowerbound = (1-tolerance)*(hemispheres[i].momentumx)
        upperbound = (1+tolerance)*(hemispheres[i].momentumx)
        for k in range (len(newevents)):
            for l in range (0,1):
               if i == newevents[k][l]:
                 flag = 1
                 continue
        ...
        for j in range (len(hemispheres)):
            if i == j:
               continue
            if hemispheres[j].momentumx <= upperbound and hemispheres[j].momentumx >= lowerbound:
               a = [i,j]
               newevents.append(a)
    # Print the array of new events
    for i in range (len(newevents)):
        print("Event {0} has hemispheres with momenta {1} and {2}".format(i, hemispheres[newevents[i][0]].momentumx, hemispheres[newevents[i][1]].momentumx))
        print("Event {0} has hemispheres with number of jets {1} and {2}".format(i, hemispheres[newevents[i][0]].numjets, hemispheres[newevents[i][1]].numjets))
